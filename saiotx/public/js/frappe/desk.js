...
frappe.Application = Class.extend({
	...
	startup: function() {
		...
		this.setup_profile_template();
	},

	setup_profile_template(){
		let profile = frappe.boot.profile;
		let user_info = frappe.user_info();
		let user_profile_path = 'desk#user-profile'
		let user_settings_path = `desk#Form/User/${frappe.session.user}`;
		
		let profile_fullname = frappe.session.user_fullname;
		let user_id = frappe.session.user_email;
		let profile_image = user_info.image;
		let profile_leaves = 0;
		let designation = "";
		let department = "";
		
		if (profile){
			if(profile.employee_name){
				profile_fullname = profile.employee_name;
			}
			if(profile.user_id){
				user_id = frappe.boot.profile.user_id;
			}
			if(profile.image){
		...
		if (!designation){
			if(frappe.user.has_role("System Manager")){
				designation = __("Administrator")
			}else{
				designation = __("Employee")
			}
		}

		if (!profile_image){
			profile_image = "/assets/saiotx/images/profile.jpg"
		}
		
		let sales_invoice_template = "";
		let purchase_invoice_template = "";
		if(frappe.user.has_role("Sales User") || frappe.user.has_role("Sales Manager")){
			//TODO get sales invoice counter
			let sales_invoice_counter = 0;
			sales_invoice_template = `
			<div class="skills sales-invoice">
				<h6>${__("Sales Invoice")}</h6>
				<ul>
					<li>${__("You have")} <span id="sales-invoices-available">${sales_invoice_counter}</span> ${__("Sales Invoice")}. </li>
				</ul>
			</div>
			`;
		}
		if(frappe.user.has_role("Purchase User") || frappe.user.has_role("Purchase Manager")){
			//TODO get sales invoice counter
			let purchase_invoice_counter = 0;
			purchase_invoice_template = `
			<div class="skills purchase-invoice">
				<h6>${__("Purchase Invoice")}</h6>
				<ul>
					<li>${__("You have")} <span id="purchase-invoices-available">${purchase_invoice_counter}</span> ${__("Purchase Invoice")}. </li>
				</ul>
			</div>
			`;
		}
		...
		let profile_template = `
			<div class="card-container">
				<span class="pro">${__(designation)}</span>	
				<img src="${profile_image}" alt="${__("Profile Image")}"/>
				<h3>${profile_fullname}</h3>
				<h6>${user_id}</h6>
				<p>${__(department)} <br/> </p>
				<div class="buttons">
					<button class="primary">
						<a href="${user_profile_path}">Profile</a>
					</button>
					<button class="primary ghost">
						<a href="${user_settings_path}">Settings</a>
					</button>
				</div>
				${purchase_invoice_template}
				${sales_invoice_template}
				<div class="skills">
					<h6>${__("Attendance")}</h6>
					<ul>
						<li>${frappe.datetime.get_today()}</li>
						<br/>
						<li style="visibility: hidden;">${frappe.datetime.get_today()}</li>
						<li>${__("Check In")}</li>
						<li id="check-in-field">${__("None")}</li>
						<br/>
						<li style="visibility: hidden;">${frappe.datetime.get_today()}</li>
						<li>${__("Check Out")}</li>
						<li id="check-out-field">${__("None")}</li>
					</ul>
				</div>
			</div>
		...
		$('#profile-info').append(profile_template);

		frappe.db.count("Sales Invoice").then(count=>{$('#sales-invoices-available')[0].innerHTML = count})
		frappe.db.count("Purchase Invoice").then(count=>{$('#purchase-invoices-available')[0].innerHTML = count})

		frappe.call({
			method:"saiotx.api.get_employee_info",
			callback:function(result){
				if(result.message){
					let info = result.message;
					for (let i = 0; i < info.length; i++) {
						const check_info = info[i];
						if(check_info.log_type == "IN"){
							$('#check-in-field')[0].innerHTML = check_info.time.split(" ")[1];
						}
						if(check_info.log_type == "OUT"){
							$('#check-out-field')[0].innerHTML = check_info.time.split(" ")[1];
						}
					}
				}
			}
